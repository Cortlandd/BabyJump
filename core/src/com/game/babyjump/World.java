...
public class World {
    ...
    private void generateLevel () {
        ...
        float maxJumpHeight = Baby.BOB_JUMP_VELOCITY * Baby.BOB_JUMP_VELOCITY / (2 * -gravity.y);
    ...
    public void update (float deltaTime, float accelX) {
        ...
        if (bob.state != Baby.BOB_STATE_HIT) checkCollisions();
        checkGameOver();
    }

    private void updateBob (float deltaTime, float accelX) {
        if (bob.state != Baby.BOB_STATE_HIT && bob.position.y <= 0.5f) bob.hitPlatform();
        if (bob.state != Baby.BOB_STATE_HIT) bob.velocity.x = -accelX / 10 * Baby.BOB_MOVE_VELOCITY;
